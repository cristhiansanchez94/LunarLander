...
python examples/agents/keyboard_agent.py LunarLauncher-v2
...
OBJECTIVE_POS_X =VIEWPORT_W/SCALE/2
OBJECTIVE_POS_Y = 12#VIEWPORT_W/SCALE*(3/4)
...
class LunarLauncher(gym.Env, EzPickle):
    metadata = {
    ...
    def reset(self):
        ...
        LANDER_INIT_X = (self.helipad_x1+self.helipad_x2)/2
        LANDER_INIT_Y = self.helipad_y
        ...
        initial_y = LANDER_INIT_Y
        self.lander = self.world.CreateDynamicBody(
            position=(LANDER_INIT_X, LANDER_INIT_Y),
            angle=0.0,
            fixtures = fixtureDef(
        ...
        for i in [-1, +1]:
            leg = self.world.CreateDynamicBody(
                position=(LANDER_INIT_X - i*LEG_AWAY/SCALE, initial_y),
                angle=(i * 0.05),
                fixtures=fixtureDef(
    ...
    def step(self, action):
        ...
        state = [
            (pos.x - OBJECTIVE_POS_X) / (OBJECTIVE_POS_X),
            (pos.y - (OBJECTIVE_POS_Y+LEG_DOWN/SCALE)) / (OBJECTIVE_POS_Y/2),
            vel.x*(OBJECTIVE_POS_X)/FPS,
            vel.y*(OBJECTIVE_POS_Y)/FPS,
            self.lander.angle,
        ...
        shaping = \
            ...
            - 100*abs(state[4]) -50*state[6]  -50*state[7]  # And ten points for legs contact, the idea is if you
        ...
        if self.game_over or abs(state[0]) >= 1.0:
            ...
            reward = -100
        if not self.lander.awake or np.sqrt(state[0]*state[0] + state[1]*state[1])<=0.1:
            done = True
            reward = +100
...
class LunarLauncherContinuous(LunarLauncher):
    continuous = True
